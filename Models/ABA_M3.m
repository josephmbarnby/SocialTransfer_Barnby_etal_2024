...
function[F, results] = ABA_noinsert_conv_Gen(parms, data, sim)
...
% Validate input parameters
   if length(parms) ~= 8
       error('Parameter vector should have 8 elements.');
   end
   if size(data, 2) ~= 9
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    ag = 0:(inc/2):res;
    bg = -30:inc:res;
    mu_alphapar    = calculate_mean(ag, alpha_marg2b);
    mu_betapar     = calculate_mean(bg, beta_marg2b);
    sigma_alphapar = calculate_std( ag, alpha_marg2b, mu_alphapar);
    sigma_betapar  = calculate_std( bg, beta_marg2b, mu_betapar);

    %convolve two probability distributions for third phase parameters
    [convolve_alpha_v, convolve_alpha_m] = convolve_distributions(alpha, alpha_v, alpha_v_ref, sigma_alphapar, mu_alphapar);
    ...
    pabg_shift=...
        (normpdf(alpha_grid,convolve_alpha_m,convolve_alpha_v).*...
    ...
    alpha_marg3 = squeeze(sum(pabg_shift,[1 3]));
    beta_marg3  = squeeze(sum(pabg_shift,[2 3]))';
